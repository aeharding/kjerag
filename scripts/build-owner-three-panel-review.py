...
def copy_canonical(path: Path, expected_sha: str, destination: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Refusal(f"cannot pin {label}: {error}") from error
    digest = sha256_bytes(raw)
    require(digest == expected_sha, f"{label} is not the canonical pinned version: expected {expected_sha}, got {digest}")
    destination.parent.mkdir(parents=True, exist_ok=True)
    with destination.open("xb") as output:
        output.write(raw); output.flush(); os.fsync(output.fileno())
    return {"path": os.fspath(path), "bytes": len(raw), "sha256": digest}
...
class PinnedReceipt:
    ...
    def verify(self) -> None:
        ...
        require(sha256_bytes(os.pread(self.fd, self.state.st_size, 0)) == self.sha256,
...
def validate_kjerag(receipt: PinnedReceipt, stage: Path) -> dict[str, Any]:
    value = receipt.value
    ...
    require((view.get("capture_width"), view.get("capture_height")) == (WIDTH, HEIGHT),
            "Kjerag capture is not 3840x2160")
    ...
    require(build.get("dirty_at_build") is False and build.get("runtime_tracked_tree_clean") is True,
            "Kjerag receipt does not bind a clean production build")
...
def derive_studio(oracle_dir: Path, private_root: Path, kjerag: dict[str, Any]) -> tuple[PinnedReceipt, dict[str, Any]]:
...
def validate_trace(receipt: PinnedReceipt, stage: Path, kjerag_receipt: PinnedReceipt,
                   kjerag: dict[str, Any]) -> dict[str, Any]:
    value = receipt.value
    ...
    for ordinal, raw in enumerate(frames):
        ...
        out.append({"index": index, "timestamp_seconds": frame.get("timestamp_seconds"),
                    "timestamp_nanoseconds": frame.get("timestamp_nanoseconds"), **authenticated})
    return {"input_receipt": input_receipt, "view": trace_view, "source": value.get("source"),
            "build": obj(value.get("build"), "trace build"), "frames": out}
...
def tool(ffmpeg: str) -> dict[str, Any]:
    path = Path(shutil.which(ffmpeg) or ffmpeg).resolve()
    require(path.is_file(), f"ffmpeg executable does not exist: {path}")
    version = run([os.fspath(path), "-version"]).stdout.decode(errors="replace").splitlines()
    require(version, "ffmpeg did not print a version")
    encoders = run([os.fspath(path), "-hide_banner", "-encoders"]).stdout.decode(errors="replace")
    require("libx264" in encoders, "ffmpeg does not provide libx264")
    return {"path": os.fspath(path), "bytes": path.stat().st_size, "sha256": sha256_path(path), "version": version}


def executable_identity(path: Path) -> dict[str, Any]:
    resolved = path.resolve()
    return {"path": os.fspath(resolved), "bytes": resolved.stat().st_size, "sha256": sha256_path(resolved)}


def probe_video(ffprobe: str, path: Path, expected_frames: int, expected_duration: str) -> dict[str, Any]:
...
def font(path: Path) -> dict[str, Any]:
    require(path.is_absolute() and path.is_file() and not path.is_symlink(), "font must be an absolute regular non-symlink file")
    digest = sha256_path(path)
    require(digest == FONT_SHA256, f"font SHA-256 differs from the historical Noto Sans input: expected {FONT_SHA256}, got {digest}")
    return {"path": os.fspath(path), "bytes": path.stat().st_size, "sha256": digest}
...
def commands(ffmpeg: str, inputs: Path, outputs: Path, font_path: Path) -> tuple[list[str], list[str]]:
    ...
    native = [ffmpeg, "-hide_banner", "-loglevel", "error",
    ...
    quarter = [ffmpeg, "-hide_banner", "-loglevel", "error", "-i", os.fspath(outputs / NATIVE),
...
def rename_noreplace(source: Path, destination: Path) -> None:
    ...
    result = libc.renameat2(AT_FDCWD, os.fsencode(source), AT_FDCWD, os.fsencode(destination), RENAME_NOREPLACE)
    if result != 0:
        error = ctypes.get_errno()
        if error in (errno.EEXIST, errno.ENOTEMPTY):
            raise Refusal(f"destination already exists: {destination}")
...
def build(args: argparse.Namespace) -> Path:
    ...
    destination.parent.mkdir(parents=True, exist_ok=True)
    receipts = [PinnedReceipt(args.kjerag_receipt, args.kjerag_sha256, "Kjerag receipt"),
                PinnedReceipt(args.studio_receipt, args.studio_sha256, "Studio receipt"),
                PinnedReceipt(args.trace_receipt, args.trace_sha256, "range-trace receipt")]
    try:
        ffmpeg = tool(args.ffmpeg)
        ffprobe_path = Path(shutil.which(args.ffprobe) or args.ffprobe).resolve()
        require(ffprobe_path.is_file(), f"ffprobe executable does not exist: {ffprobe_path}")
        ffprobe = {**executable_identity(ffprobe_path),
                   "version": run([os.fspath(ffprobe_path), "-version"]).stdout.decode(errors="replace").splitlines()}
        font_input = font(args.font)
        with tempfile.TemporaryDirectory(prefix=".three-panel-inputs-", dir=destination.parent) as temporary:
            inputs = Path(temporary)
            for name in ("kjerag", "studio", "trace"): (inputs / name).mkdir()
            stage = Path(tempfile.mkdtemp(prefix=f".{destination.name}.three-panel-stage-", dir=destination.parent))
            try:
                kjerag = validate_kjerag(receipts[0], inputs / "kjerag")
                studio = validate_studio(receipts[1], inputs / "studio", kjerag)
                derived_receipt, derivation = derive_studio(args.oracle_dir, inputs / "canonical-projector", kjerag)
                try:
                    (inputs / "derived-copy").mkdir()
                    derived = validate_studio(derived_receipt, inputs / "derived-copy", kjerag, CANONICAL_DERIVED_AUTH)
                    compare_studio_derivation(studio, derived)
                    derived_receipt.verify()
                finally:
                    derived_receipt.close()
                trace = validate_trace(receipts[2], inputs / "trace", receipts[0], kjerag)
                native, quarter = commands(ffmpeg["path"], inputs, stage, args.font)
                run(native)
                run(quarter)
                outputs = {
                    "native": {**file_identity(stage / NATIVE),
                               "ffprobe": probe_video(ffprobe["path"], stage / NATIVE, COUNT, "2.035367")},
                    "quarter_speed": {**file_identity(stage / QUARTER),
                                      "ffprobe": probe_video(ffprobe["path"], stage / QUARTER, COUNT * 4, "8.141467")},
                }
                for path in (stage / NATIVE, stage / QUARTER): sync_file(path)
                for pinned in receipts: pinned.verify()
                record = {
                    ...
                    "inputs": {
                        "receipts": {"kjerag": receipts[0].identity(), "studio": receipts[1].identity(), "range_trace": receipts[2].identity()},
                        "kjerag": {"source": kjerag["source"], "build": kjerag["build"], "run": kjerag["run"], "frames": kjerag["frames"]},
                        "studio": studio,
                        "studio_private_derivation": derivation,
                        "range_trace": trace,
                        "font": font_input,
                    },
                    "tools": {"ffmpeg": ffmpeg, "ffprobe": ffprobe,
                              "python": {**executable_identity(Path(sys.executable)), "version": sys.version},
                              "builder": executable_identity(Path(__file__))},
                    "commands": {"native": native, "quarter_speed": quarter},
                    "outputs": outputs,
                    "historical_reproduction": {
                        ...
                        "historical_derivatives_were_receipted": False,
                        "explicit_font_added": True,
                ...
                receipt_path = stage / OUTPUT_RECEIPT
