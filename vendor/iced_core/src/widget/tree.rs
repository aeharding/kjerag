...
impl Tree {
    ...
    pub fn diff_children_custom<T>(
        ...
        let mut id_list = VecDeque::with_capacity(old_children.len());

        for (i, child) in old_children.iter().enumerate() {
            if let Some(Id(Internal::Custom(_, name))) =
                child.as_ref().and_then(|child| child.id.as_ref())
            {
                let _ = id_map.insert(name.to_string(), i);
            } else {
                id_list.push_back((i, i));
            }
        }

        self.children.reserve(new_children.len());

        for (i, (new, new_id)) in
            new_children.iter_mut().zip(new_ids.iter()).enumerate()
        {
            let old_index = new_id
                .as_ref()
                .and_then(|id| {
                ...
                .or_else(|| {
                    ...
                    found.and_then(|found| {
                        id_list.remove(found).map(|(_, old)| old)
                    })
                });

            let mut child_state = old_index
                .and_then(|old| old_children.get_mut(old))
                .and_then(Option::take)
                .unwrap_or_else(|| new_state(new));

            diff(&mut child_state, new);
            self.children.push(child_state);
        }
    }
}
