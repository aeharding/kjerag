...
use kjerag_spike::{FORMAT, Gpu, Picture, Render, Seam as SeamArg};
...
fn main() -> Fallible<()> {
    ...
    // The app's own path, and the only thing that separates the two arms of
    // the delivered table: the session's own across-seam term, harvested off
    // this file's frames and walked in as far as its own seam accepts it.
    // Synchronous here, because a still has no later moment to correct itself
    // in (`Scene::learn_epi`).
    if options.epi {
        scene.learn_epi();
    }
...
struct Options {
    ...
    window: (f64, f64),
    seam: SeamArg,
    /// Whether this arm harvests the session's own across-seam term the way
    /// the app does, which is the only difference between the two arms of the
    /// delivered table.
    epi: bool,
...
impl Options {
    fn parse(args: impl Iterator<Item = String>) -> Fallible<Self> {
        ...
        // file and the file may be named anywhere on the line.
        options.seam = SeamArg::parse(&seam, &options.input)?;
        Ok(options)
    }
...
const USAGE: &str = "usage: epiramp <file.insv> [time=seconds] [warm=seconds] [yaw=deg] \
     ...
     [window=low,high] [epi=0] [seam=factory|file|pool|roll:0.6,yaw:-2.1,pitch:-0.9,cx:-9.5,cy:-11.9] \
