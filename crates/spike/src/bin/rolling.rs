...
/// One frame of every lens, in system memory.
struct Pair {
    index: u64,
    at: Duration,
    lenses: Vec<Plane>,
...
/// One lens's luma plane, as the decoder handed it over.
struct Plane {
    luma: Vec<u8>,
    stride: usize,
    size: MetaSize,
...
impl Plane {
    fn of(frame: &SwFrame, size: MetaSize) -> Self {
...
/// A walk forward through both lens streams from one instant, delivering
/// frames in pairs.
///
/// The player's own [`kyerag_media::Reader`] does this into GPU memory, which
/// is where the app wants frames and not where this instrument does.
struct Walk {
...
impl Walk {
    fn open(path: &PathBuf, from: f64, size: MetaSize) -> Fallible<Self> {
        ff::init()?;
        ...
        let stream = input.stream(streams[0]).ok_or("no video stream")?;
    ...
    /// The next instant both lenses have a frame for, at or after the one the
    /// walk was opened on.
    fn next_pair(&mut self) -> Fallible<Option<Pair>> {
        loop {
