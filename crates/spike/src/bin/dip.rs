...
/// window there is.
///
/// The first line is the whole of the initialisation: `Filter::solve` starts
/// from whichever tilt puts the first tenth of a second of accelerometer
/// readings on the world vertical, and it does that **whatever that tenth of
/// a second reads**, where the running filter would refuse a reading that far
/// from 1 g outright.
fn settling(calibration: &CalibrationSet) -> Fallible<()> {
    ...
    // What the initialisation was handed: the first tenth of a second, which
    // is `level`'s own window.
    let first = (0.1 * rate) as usize;
    let seed = samples
        .iter()
        .take(first.max(1))
        .fold([0.0; 3], |held, sample| {
            std::array::from_fn(|axis| held[axis] + to_body.mul_vec(sample.accel_g)[axis])
        });
    let seed = seed.map(|axis| axis / first.max(1) as f64);
    let filter = Filter::default();
    println!(
        "init:   the first 0.1 s reads {:.3} g, and the filter starts from it whatever it 
         	reads. The running filter believes a reading completely only inside {:.2} g of 
         	1 g and not at all outside {:.2}, so this seed is one it would refuse.",
        norm(seed),
        filter.trust_g.0,
        filter.trust_g.1,
    );
