...
//!
//! An arm is `<label>=<basis>+<pose>`, and the pose is `none`, `pool`, or five
//! knobs written out. The basis is one of:
//!
//! - `v3`, what every build before `hack/flat-v6` drew;
//! - `v6pose`, v6's eleven pose and intrinsic tokens - which sit in the same
//!   slots in both grammars - over v3's five-coefficient distortion;
//! - `v6:<reading>`, all thirteen of v6's distortion run under one named
//!   [`kjerag_meta::Reading`], e.g. `v6:tang=p1p2.grow=p2p1.prism=xyxy.dir=inv`
//!   or the shorthands `v6:branch` and `v6:plain`.
//!
//! **The reading is a parameter because the branch's own was refuted.** The
//! form of the model is not in doubt - it is the disassembly of
//! `OmniProjection<RadtanDistortPro>` - but the mapping from the string's
//! thirteen token positions onto that form was assumed, and so was the
//! direction the polynomial is evaluated in. Those are four binary choices,
//! sixteen readings, and `Reading::every()` is all of them. What this binary
//! adds over running the app sixteen times is that every reading is measured
//! at ONE set of sites, so a calibration that moves the crossover cannot also
//! move the content being matched.
//!
//! **`search=` decides whether a reading of this is worth anything.** The
//! default 1.40 degrees is `--bin crossing`'s, and it is a ceiling on what can
//! be reported: a pure-factory arm on this camera disagrees by more than that
//! at many sites, so the coarse search saturates and every arm's worst column
//! comes back the same number. The 2026-08-08 sweep ran `search=3.00`, at
//! which the read rate roughly doubles and the worst column spreads out.
...
use kjerag_meta::{CalibrationSet, Filter, Lens, Quat, Reading as V6Reading};
...
fn main() -> Fallible<()> {
    ...
    let arms = options
        .arms
        .iter()
        .map(|arm| arm.build(&options.input, &written, &v3, v6.as_ref()))
...
fn say_v6(written: &offset::Written, v3: &[Lens]) -> Fallible<Option<offset::Blocks>> {
    ...
    println!(
        "  all thirteen reach every v6 arm below. Which slot carries which coefficient, and \
         which way the polynomial runs, is the candidate: an arm is named v6:<reading> and \
         the reading is `kjerag_meta::Reading`, the same mapping the app draws through."
    );
    Ok(Some(read))
}
...
impl Asked {
    ...
    fn build(
        ...
        v3: &[Lens],
        v6: Option<&offset::Blocks>,
    ) -> Fallible<Arm> {
        let missing = "this file writes no offset_v6, so no v6 arm can be built";
        let lenses = match self.basis.as_str() {
            "v3" => v3.to_vec(),
            // v6's eleven pose and intrinsic tokens with v3's distortion run:
            // the honest subset, and the arm that separates a pose change
            // from a distortion change.
            "v6pose" => {
                v6.ok_or(missing)?
                    .lenses(written.dimension, written.crop, Carry::From(v3))?
            }
            other => {
                let spec = other
                    .strip_prefix("v6:")
                    .ok_or_else(|| format!("no calibration basis called {other}"))?;
                let reading = V6Reading::parse(spec)?;
                v6.ok_or(missing)?
                    .lenses(written.dimension, written.crop, Carry::Full(reading))?
            }
        };
