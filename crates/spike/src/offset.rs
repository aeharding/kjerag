...
//!
//! **All thirteen fit now.** [`kjerag_meta::Distortion`] carries a [`Pro`] and
//! the shader's `LensBlock` carries its eight terms and a flag, so a v6 block
//! can be loaded whole. What is still a choice is which token position carries
//! which coefficient and which way the polynomial runs, and that is
//! [`kjerag_meta::Reading`] - sixteen of them, all measured. So
//! [`Blocks::lenses`] still takes a [`Carry`], and no arm this module builds
//! is allowed to call itself v6 without saying which reading it used.
//!
//! **The 2026-08-08 verdict, so nothing reads this module and re-guesses.**
//! Every one of the sixteen readings was scored ring-wide on the along-seam
//! axis, on three captures and five disjoint windows of one of them. None of
//! them closes the seam better than `offset_v3`. The half of v6 that IS worth
//! drawing is the eleven pose and intrinsic tokens: `Carry::From(v3)` on a v6
//! block, which the app spells `KJERAG_OFFSET=v6pose`, and which is worth
//! about half a degree across the seam on every file tried.

use std::io::{Read, Seek, SeekFrom};
use std::path::Path;

use kjerag_media::Fallible;
use kjerag_meta::{Distortion, Intrinsics, Lens, Pose, Reading, Size};
...
pub enum Carry<'a> {
    ...
    Written,
    /// All thirteen of a v6 block, under one named [`Reading`] of which slot
    /// carries which coefficient and which way the polynomial runs.
    ///
    /// The reading is a parameter and not a constant because the branch's own
    /// was refuted by the owner's eye and had never been measured. The
    /// mapping itself is [`kjerag_meta::name_v6`], which is also what the app
    /// draws through, so an arm scored here and a picture drawn later cannot
    /// be reading two different strings.
    ///
    ...
    fn of(self, block: &Block, index: usize) -> Fallible<Distortion> {
        match self {
            ...
            Self::Full(reading) => {
                ...
                Ok(kjerag_meta::name_v6(run, reading))
            }
