...
/// gated.
///
/// `to` is exclusive and `gate` is the plausibility test [`seam::gated`]
/// applies; the control that turns it off says whether a result belongs to the
/// gate or to the reduction.
pub fn field(rows: &[Row], from: f64, to: f64, middle: Middle, gate: bool) -> Vec<At> {
    ...
    let kept = seam::gated(reduced.iter().map(At::leftover).collect());
...
fn error_of(all: &[f64], middle: Middle) -> f64 {
    let (spread, count) = match middle {
        ...
        Middle::Median => (MAD_TO_SIGMA * mad(all), all.len()),
        Middle::Trimmed => {
            let kept = kept_of(all);
