...
/// Research only: whether the gate on the way out is filtered, from
/// `KJERAG_TRUST`. `smooth` is the only value it takes; anything else, and the
/// unset every shipped run has, applies the gate instantaneously as it ships.
///
/// **The defect it is aimed at** (docs/research/seam-temporal.md 2.2, C1a).
/// The state is smoothed and the gate on the way out is not: what reaches the
/// picture is [`Cell::disparity`] times `clamp(confidence / KEEP, 0, 1)` read
/// this frame. On the owner's May-01 downward arc the held disparity sat at
/// -0.912 degrees for four seconds while the applied value swung between 0.00
/// and -47.61 view pixels, **84 frame-to-frame steps over 10 view px and a
/// worst of 46.74**, because the content flickers the correlation and the gate
/// follows it whole.
///
...
const TRUST_FILTER: &str = "KJERAG_TRUST";

/// What `KJERAG_TRUST` has to say to get the filtered gate.
...
/// design claim in C1a.** [`time_constant`] runs a direction fast when its
/// disparity is near-field sized, because *the wing moves* and a near reading
/// has to track it. The correlator losing the scene is not the wing moving: it
/// is the same content, still there, momentarily not correlating. Those two
/// want opposite responses and today they share a knob. So the value follows
/// the wing at 0.1 s and the belief in it fades at 2, and a direction that
/// stops confirming its reading **fails towards the reading it held** instead
/// of towards nothing.
///
...
///
/// **Falsifiable, and the A/B is what falsifies it**: if a slow gate smears
/// the wing, the owner will see it at the near content and this is refused.
const TAU_TRUST_S: f32 = 2.0;

/// Whether this run filters the gate on the way out, from [`TRUST_FILTER`].
///
/// Read once, like [`super::projection::blend_power`], and read by both halves
/// of the map: the Rust one from [`super::Reframe::channel`] and the shader
/// from a constant [`wgsl`] and [`lookup_wgsl`] write into it.
pub(crate) fn smooth_trust() -> bool {
    static SMOOTHED: OnceLock<bool> = OnceLock::new();
    *SMOOTHED.get_or_init(|| {
        let Ok(asked) = std::env::var(TRUST_FILTER) else {
            return false;
        };
        if asked != SMOOTH {
            eprintln!(
                "kjerag: {TRUST_FILTER}={asked} is not {SMOOTH:?}, so the band keeps the \
                 instantaneous gate it ships with"
            );
            return false;
        }
        println!(
            "band:   research trust on, {TRUST_FILTER}={SMOOTH}: what the band applies fades \
             towards the reading it holds over {TAU_TRUST_S} s instead of following the \
...
pub struct Cell {
    ...
    /// gate on the way out, stored rather than recomputed per fragment
    /// (docs/research/seam-temporal.md, C1a).
    ///
    /// It is `clamp(confidence / KEEP, 0, 1)` on either arm. What
    /// [`TRUST_FILTER`] decides is whether that is this frame's answer or one
    /// filtered at [`TAU_TRUST_S`], and this field holds whichever the run is
    /// applying - so an instrument reading it is reading the picture on both
    /// arms rather than a model of one.
    ///
...
impl Cell {
    ...
    /// none. What such a file gets is the value that arm applied,
    ...
    /// pass runs this on the GPU where no test can reach it, and what C1a
    /// claims about a confidence dropout is claimed about a function
    /// `cargo test` can call with no device and no footage.
    ///
    /// `fresh` is a reset frame, and a direction whose trust is nothing takes
    /// the answer whole for the same reason the disparity and the confidence
    /// do: there is no picture behind a direction that has never been read for
    /// an ease to hide, and creeping in from zero over two seconds would draw
...
// it. Rust twin: `Cell::trust`'s docstring and `believe`.
//
// SMOOTH_TRUST off is the shipped law: this frame's evidence, whole. On, it is
// the same quantity filtered at TAU_TRUST, so a direction that stops
// confirming its reading fades towards the reading it holds rather than
// snapping off it.
...
// evidence, then taxed by how much of that evidence reaches `KEEP`.
//
...
fn carry(a: Cell, b: Cell, mix: f32) -> f32 {
  ...
  return (ea * a.disparity + eb * b.disparity) / total * strength(a, b, mix);
}

// How much of that mixed reading reaches the picture.
//
// The two arms clamp on opposite sides of the mix and that is not a detail:
// off, one number is mixed and then clamped, which is what ships; on, each
// cell's own filtered gate is mixed, already clamped. The off branch is the
// shipped expression written out unchanged, so the null is byte-identity and
// not a tolerance. Rust twin: `Reframe::strength`.
fn strength(a: Cell, b: Cell, mix: f32) -> f32 {
  if SMOOTH_TRUST {
    return mix2(a.trust, b.trust, mix);
  }
  return clamp(mix2(a.confidence, b.confidence, mix) / KEEP, 0.0, 1.0);
}
...
//
// Off, it is this frame's evidence whole, which is the shipped expression the
// fragment shader used to compute for itself. On, it eases towards that at
// TAU_TRUST - except from nothing, which it takes whole for the same reason
// the confidence and the disparity do: a direction that has never been read
...
// It is THIS FRAME's evidence on both arms: `KJERAG_TRUST=smooth` filters what
// the bend applies and deliberately not what the exposure pools, because an
// exposure is not a geometry and nothing has measured that it wants the same
// constant.
@compute @workgroup_size(THREADS)
...
mod trust_tests {
    ...
    ///
    /// `reading` is what the correlator returns on each visit, or `None` for a
    ...
    /// The plant itself: eight visits correlating well, eight the gates refuse,
    /// eight correlating again. Nothing about the disparity moves.
    ...
    fn the_shipped_gate_snaps_a_planted_flicker() {
        ...
        assert!(
            worst > 10.0,
            "the planted flicker moves the picture by at most {worst} view px on the shipped \
             gate, so it is not the defect the trace measured"
        );
    ...
    /// out and **+0.39, -0.12, -0.27, -0.26** coming back. **None over 10, and
    /// the arrival step is the same -46.69 on both arms**, because C1a does
    /// not touch the rule that a direction with no picture behind it takes its
    /// first answer whole.
    #[test]
    fn the_filtered_gate_fades_a_planted_flicker() {
        ...
        // arms: a direction with no picture behind it has nothing for an ease
        // to hide.
