...
const RING: &str = r#"
// The research across-seam term at one direction of the ring, in radians, and
// zero at every direction unless `KJERAG_EPI_TERM` is on. Rust twin:
...
fn band_bend(ray: vec3<f32>) -> Band {
  ...
  // RESEARCH (`KJERAG_EPI_TERM`), and zero unless it is on: the same axis and
  // the same scale as the offset above, added to the term lens 1 takes WHOLE.
  // That is the experiment - one displacement of a whole picture instead of a
  // ramp across the corridor - and it is why this rides on `along` rather than
  // on `offset`, which is shared out. Rust twin: `Bend::still`.
  //
...
// The research across-seam term at one direction, the same lookup one array
...
fn measure(@builtin(workgroup_id) group: vec3<u32>, @builtin(local_invocation_index) lane: u32) {
  ...
  // slide it.
  //
  // Read THROUGH the research across-seam term, and zero unless it is on: a
  // ring read past a displacement the picture is already being drawn with
  // would ask for it a second time, and the band would then apply as a
  // corridor ramp exactly what the term had just taken out. What this pass
  // answers with the term on is what the term still leaves. Rust twin:
