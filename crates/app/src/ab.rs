...
const KNOBS: &[(&str, Swap)] = &[
    ...
    ("handover", Swap::Live),
    ("sampling", Swap::Live),
    // The belt: Studio's Optical Flow arm against Off, which is the one
    // question this harness was extended for. `Reframe::belt` is a word of the
    // uniform block and the block is rebuilt every redraw, so this is a store
    // and a redraw like the two above.
    ("flow", Swap::Live),
...
enum Knob {
    ...
    Sampling(Sampling),
    /// Whether the belt runs and the picture reads its field.
    Flow(bool),
}
...
pub fn wear(session: &Session, trial: usize, position: usize, scene: &kjerag_render::Scene) {
    for knob in session.knobs(trial, position) {
        match knob {
            ...
            Knob::Flow(on) => scene.set_flowing(*on),
        }
    }
}
    ...
    fn read(term: &str) -> Result<Self, String> {
        ...
        match KNOBS.iter().find(|(known, _)| *known == name) {
            ...
            Some((_, Swap::Live)) => match name {
                ...
                "flow" => match value {
                    "on" | "1" => Ok(Self::Flow(true)),
                    "off" | "0" => Ok(Self::Flow(false)),
                    _ => Err(format!("flow={value} is not on or off")),
                },
