...
pub struct Config {
    ...
    /// The available Studio-derived optical-flow seam correction, the
    /// "Optical Flow" arm of the stitching control (`Scene::set_flow`).
    ///
...
///
/// This held a per-camera `seam_pool` until 2026-08-15, the fits the app made
/// by watching one flight's seam and drew the next capture of that camera with.
/// That per-capture content fit was the non-parity mechanism (issue #48) and
/// was removed: the player draws the factory calibration, which needs no saved
/// state. An old state file's `seam_pool` key is left on disk unread, the way
/// `#[serde(default)]` leaves any key the struct no longer names.
